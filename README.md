...
- **Author / Student:** Sumadh Raj Gurung (Roll No. 32)
...
```text
f1_dashboard/
├── public/              # Public browser assets (favicon, manifest)
├── src/
│   ├── assets/          # F1 logo and local image assets
│   │   └── f1-logo.svg  # Dashboard header logo
│   ├── App.jsx          # Single-file dashboard UI, data loading, and interactions (~1740 lines)
...
The entire dashboard UI, data fetching, state management, and styling live in a single `App.jsx` component. There is no client-side routing; views are switched with a tab bar and detail panels are shown inline.
...
The complete student project report and documentation is available in [REPORT.md](REPORT.md), containing student details, objectives, methodology, React concepts, feature descriptions, and submission requirement checklists.
